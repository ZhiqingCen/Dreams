...
from src.user import user_profile_setemail_v2, user_profile_v2
...
def test_basic_pass(clear_data):
    ...
    user_profile_setemail_v2(login['token'], 'new.email@example.com')
...
def test_invalid_token(clear_data):
    ...
    with pytest.raises(AccessError):
        user_profile_setemail_v2('invalid_token', 'new.email@example.com')

def test_invalid_email(clear_data):
    clear_data
    login = auth_register_v2('john.apple@example.com', 'password1', 'John', 'Apple')
    with pytest.raises(InputError):
        user_profile_setemail_v2(login['token'],  'invalidemail.com')
...
def test_email_in_use(clear_data):
    ...
    with pytest.raises(InputError):
        user_profile_setemail_v2(login['token'], 'mary_jane@yahoo.com')